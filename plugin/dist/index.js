...
// ── MCP Channel Server ──────────────────────────────────────────
const server = new Server({ name: "swarm-plugin", version: "0.1.0" }, {
    capabilities: {
    ...
    try {
        switch (name) {
            case "register": {
                ...
                connectSSE(id);
                return text(`Registered as "${agentName}" (${id}). Listening for messages. Auto-reconnect enabled.`);
            }
            case "whoami": {
                if (!agentId)
                    return text("Not registered in the swarm yet. Use 'register' first.");
                const me = await api("GET", `/agents/${agentId}`);
                const connections = await api("GET", `/agents/${agentId}/connections`);
                return text(`Your swarm identity:\n${JSON.stringify(me, null, 2)}\n\nConnections:\n${JSON.stringify(connections, null, 2)}\n\nNote: You also have access to all skills, MCPs, and tools configured in your Claude Code session — these are part of your capabilities even if not listed in your swarm description.`);
            }
            case "update_profile": {
                if (!agentId)
                    return text("You must register first.");
                const updates = {};
                const a = args;
                if (a.name)
                    updates.name = a.name;
                ...
                if (Object.keys(updates).length === 0)
                    return text("Nothing to update. Provide name and/or description.");
            ...
            case "discover": {
                ...
                const { query } = args;
                const agents = await api("GET", `/agents/${agentId}/connections`);
                return text(`Search query: "${query}"\n\nConnected agents:\n${JSON.stringify(agents, null, 2)}\n\nMatch the query against the agent descriptions to find the best fit.`);
            }
            case "send_message": {
                if (!agentId)
                    return text("You must register first.");
                const { to, content } = args;
                const result = await api("POST", "/messages", { from: agentId, to, content });
                if (result.error)
                    return text(`Blocked: ${result.error}`);
    ...
    try {
        ...
        while (!signal.aborted) {
            ...
            for (const part of parts) {
                ...
                if (event === "message") {
                    await pushChannel(`Message from ${data.from}: ${data.content}`, { from: sanitizeKey(data.from), msg_id: sanitizeKey(data.id) });
                }
                else if (event === "agent_online") {
                    await pushChannel(`Agent online: ${data.name} (${data.id}) — ${data.description}`, { event_type: "agent_online", agent_id: sanitizeKey(data.id) });
                }
                else if (event === "agent_offline") {
                    await pushChannel(`Agent offline: ${data.name} (${data.id})`, { event_type: "agent_offline", agent_id: sanitizeKey(data.id) });
                }
                else if (event === "connected_to") {
                    await pushChannel(`New connection: ${data.name} (${data.id}) — ${data.description}`, { event_type: "connected_to", agent_id: sanitizeKey(data.id) });
                }
                else if (event === "disconnected_from") {
                    await pushChannel(`Connection removed: ${data.id}`, { event_type: "disconnected_from", agent_id: sanitizeKey(data.id) });
                }
                else if (event === "agent_renamed") {
                    await pushChannel(`Agent renamed: "${data.oldId}" is now "${data.newId}" (${data.name}). Use the new ID for future messages.`, { event_type: "agent_renamed", old_id: sanitizeKey(data.oldId), new_id: sanitizeKey(data.newId) });
                }
                    ...
                    await pushChannel(`Your properties were updated by the swarm admin. No action required. New values — ID: "${data.id || agentId}", Name: "${data.name}", Description: "${data.description}".${data.publicDescription ? ` Public description: "${data.publicDescription}".` : ""}`, { event_type: "properties_updated" });
                }
            }
        }
    }
...
async function autoRegister() {
    ...
    try {
        ...
        if (connectRes.ok) {
            ...
            await pushChannel(`Reconnected to swarm as "${data.agent.name}" (${data.agent.id}). ${data.agent.description} ${connList} If your capabilities have changed (new skills, MCPs, etc.), use update_profile to update your swarm description.`, { event_type: "auto_connected", agent_id: sanitizeKey(config.id) });
        }
        else if (connectRes.status === 404) {
            // ID not in service — agent needs to register manually
            await pushChannel(`Auto-connect failed: agent "${config.id}" is not known to the swarm service. Use 'register' to register with a full description of your capabilities.`, { event_type: "auto_connect_failed" });
        }
